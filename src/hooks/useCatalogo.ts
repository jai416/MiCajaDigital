  ...
  const getAll = useCallback(
    async (limit: number = 0, offset: number = 0): Promise<CatalogoItem[]> => {
      try {
        ...
        if (!userId) return [];
        const sql = 'SELECT * FROM catalogo WHERE user_id = ? ORDER BY nombre ASC' +
    ...
    async (query: string): Promise<CatalogoItem[]> => {
      ...
      try {
        return await db.getAllAsync<CatalogoItem>(
          `SELECT catalogo.* FROM catalogo JOIN catalogo_fts ON catalogo.rowid = catalogo_fts.rowid WHERE catalogo_fts MATCH ? AND catalogo.user_id = ? ORDER BY catalogo.nombre ASC LIMIT ${BUSCAR_LIMIT}`,
          [ftsQuery, userId]
        );
      } catch {
        const pattern = `%${query}%`;
        return await db.getAllAsync<CatalogoItem>(
          'SELECT * FROM catalogo WHERE user_id = ? AND (nombre LIKE ? OR descripcion LIKE ?) ORDER BY nombre ASC LIMIT ?',
