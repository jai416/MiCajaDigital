...
async function getStats(dias: number) {
  ...
  // ---- RPC: todos los conteos base en 1 sola query ----
  const { data: rpc, error: rpcError } = await supabaseAdmin
    .rpc('stats_dashboard_v2', { p_dias: dias })
    .single();

  if (rpcError || !rpc) {
    fallos.push('stats_dashboard_v2 (RPC)');
  }

  const rpcData = (rpc ?? {}) as Record<string, unknown>;
  const neg = (rpcData.negocios ?? {}) as Record<string, number>;
  const cod = (rpcData.codigos ?? {}) as Record<string, number>;
  ...
  const gastosRangoPrev = (rpcData.gastos_prev as number) ?? 0;
  const codigosGenerados = cod.generados ?? 0;
  const codigosUsados = cod.usados ?? 0;
  const codigosPorVencer = cod.por_vencer ?? 0;
  const codigosVencidosSinUsar = cod.vencidos_sin_usar ?? 0;
  const nuevos = (rpcData.nuevos as number) ?? 0;
  ...
  const negocios = (rNegocios.data ?? []) as Array<{
    id: string; activo: boolean | null; plan: string | null;
    fecha_registro: string | null; fecha_expiracion: string | null;
    deleted_at: string | null; tc_usd?: number | null; tc_mlc?: number | null;
    nombre_negocio?: string | null; email?: string | null;
  }>;
  ...
  const ingresosReales = rIngresosMeses.data ?? [];

  // ---- Negocios: estados ----
  const vivos = negocios.filter((n) => !n.deleted_at);
  interface RowNeg {
    id: string; activo: boolean; plan: string; registro: number; expiracion: number;
  }
  ...
  for (let i = dias - 1; i >= 0; i--) {
    ...
    const totalDia = rows.filter((n) => n.registro >= inicio && n.registro < inicio + MS_DIA).length;
      ...
      <div className="bg-white rounded-xl shadow-sm border border-orange-200 p-6 mb-8">
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
          ...
          <a href="/dashboard/negocios" className="group bg-yellow-50 hover:bg-yellow-100 transition rounded-lg p-4">
            ...
            <p className="text-[11px] text-yellow-400 mt-1 group-hover:text-yellow-500">Momento de ofrecer el plan</p>
          </a>
        </div>
      </div>
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mt-5">
            ...
            <div className="bg-sky-50 rounded-lg p-4">
              <p className="text-xl font-bold text-sky-700">
                {Object.entries(stats.monedaResumen).map(([m, n]) => `${m}: ${n}`).join(' · ') || '—'}
              </p>
